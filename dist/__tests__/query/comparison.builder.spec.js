...
describe('ComparisonBuilder', () => {
    const createComparisonBuilder = () => new query_1.ComparisonBuilder();
    ...
    describe('notBetween comparisons', () => {
        ...
        it('should throw an error if the comparison is not a between comparison', () => {
            const between = [1, 10];
            expect(() => createComparisonBuilder().build('numberType', 'notBetween', between)).toThrow('Invalid value for not between expected {lower: val, upper: val} got [1,10]');
        });
    });
});
